...
import { userActionFail, userActionStart, userList, userLogin, userRegister } from "../slices/users";
...
export const registerUser = (userForm) => async (dispatch, getState) => {
  try {
    ...
    dispatch(userActionStart());
    const { data } = await axios.post(`${END_POINT}/users/register`, userForm, config);
