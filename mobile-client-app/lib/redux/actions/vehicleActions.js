...
import { deleteVehicleSuccess, getVehiclesSuccess, getVehicleSuccess, registerVehicleSuccess, updateVehicleSuccess, vehicleActionFail, vehicleActionStart } from "../slices/vehicleSlices";
...
export const getVehicleById = (vehicleId) => async (dispatch, getState) => {
  try {
    ...
    dispatch(vehicleActionStart());
    const { data } = await axios.get(`${END_POINT}/vehicles/${vehicleId}`, config);
...
export const updateVehicle = (vehicleId, vehicleData) => async (dispatch, getState) => {
  try {
    const {
      user: { userData },
    } = getState();
