...
const VehicleForm = ({
  ...
  const {loaders, profile: driver} = useSelector((state) => state.user);
  ...
  useEffect(() => {
     if (initialData?.currentDriver) {
       dispatch(getUserById(initialData?.currentDriver)); 
     }
      if (initialData?.currentLoaders?.length) {
        dispatch(getLoadersByIds(initialData.currentLoaders)); 
      }
  }, [
